...
static void init (void) {
	...
	UNSETBIT(TMR32B0MCR, 9);  // Enable interrupt MR3
	SETBIT(TMR32B0MCR, 10);  // Disable reset MR3
	...
	SETBIT(LPC_GPIO1_DIR, 0);
}

static void pin_low(void) {
	SETBIT(LPC_GPIO1_DATA, 0);
}

static void pin_high(void) {
	UNSETBIT(LPC_GPIO1_DATA, 0);
}

static void pin_flip(void) {
	FLIPBIT(LPC_GPIO1_DATA, 0);
}
